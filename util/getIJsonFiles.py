...
if TLoc.find("DEV") > -1:
	...
	dirPFX = ""
usefulBit = f"""{dirPFX}{TDS.nowStr(TDS.DT.now())}.useful.txt"""
moreUsefulBit = f"""{dirPFX}{TDS.nowStr(TDS.DT.now())}.moreUseful.txt"""
...
""")
	...
	with open(filename, "r") as FDIDIn:
		while True:
			...
			filename = f"""{dirPFX}ID{thisID}.DEETSL.{TDS.nowStr(TDS.DT.now())}.geojson"""
